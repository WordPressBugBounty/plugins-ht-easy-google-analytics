...
class Recommended_Plugins {
	...
	public function __construct( $args ) {
		...
		$this->tab_list         = ! empty( $args['tab_list'] ) ? $args['assets_url'] : array();

		add_action( 'admin_menu', array( $this, 'admin_menu' ), $this->priority );
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_assets' ) );

		// Ajax Action.
		add_action( 'wp_ajax_' . $this->text_domain . '_ajax_plugin_activation', array( $this, 'plugin_activation' ) );
	 ...
	 * [add_new_tab]
	 *
	 * @param array $tab_list  set tab content.
	 */
									...
									<div class="plugin-card htrp-plugin-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
										<div class="plugin-card-top">
											<div class="name column-name" style="margin-right: 0;">
												<h3>
													<a href="<?php echo esc_url( $details_link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo esc_html( $modal_class ); ?>>
										...
										if ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && $plugins_type == 'pro' ) {
											...
											<div class="column-downloaded">
												<a href="<?php echo esc_url( $details_link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo esc_html( $modal_class ); ?>><?php echo esc_html__( 'More Details', 'ht-easy-ga4' ); ?></a>
												<span class="downloaded-count">
												<?php
												if ( $plugins_type == 'free' ) {
													$count = $this->active_install_count( $prepare_plugin[ $data['slug'] ]['active_installs'] );
	...
	public function get_plugins( $username = 'htplugins' ) {
		...
		if ( false === $org_plugins_list ) {
			$plugins_list_by_author = plugins_api(
				'query_plugins',
				array(
					'author'   => $username,
					'per_page' => 100,
